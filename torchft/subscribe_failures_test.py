...
class TestSubscribeFailures(TestCase):
    def test_subscribe_failures_notification_manager_failure(self) -> None:
        ...
        try:
            ...
            manager_quorum_timeout = timedelta(milliseconds=server_opt["join_timeout_ms"] * 3)
            manager_heartbeat_interval = timedelta(milliseconds=server_opt["heartbeat_timeout_ms"] // 3)
            ...
            if manager_B._lighthouse_client is not None:
                manager_B_failure_stream = manager_B._lighthouse_client.subscribe_failures(
            ...
            thread_A = threading.Thread(target=manager_A.start_quorum, args=(manager_quorum_timeout,))
            thread_B = threading.Thread(target=manager_B.start_quorum, args=(manager_quorum_timeout,))
            ...
            sleep_duration_stage2 = (
                server_opt["heartbeat_timeout_ms"] +
                server_opt["failure_tick_ms"] * 2
            ) / 1000.0
            ...
            if manager_B_failure_stream is not None:
                try:
                    ...
                    assert "repA" in failure_note.replica_id, \
            ...
            assert manager_B.num_participants() == 1, \
